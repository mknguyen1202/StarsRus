...
class Withdraw extends Component {
    ...
    componentDidMount() {
        ...
        if (withdraw_id) {
            this.findByWithdrawID(withdraw_id);
        }
    }

    findByWithdrawID = (withdraw_id) => {
        WithdrawService.editWithdraw(withdraw_id)
        .then(response =>  {
            if (response.data != 0) {
                this.setState({
    ...
    add_withdraw_button(event) {
        ...
        WithdrawService.createWithdraw(newwithdraw)
        .then(response => {
            if (response.data != 0) {
                this.setState(this.initialState);
                alert("Withdraw Added Successfully!");
            } else {
        ...
        return (
            ...
            <Form onSubmit={this.add_withdraw_button} id="withdrawFormId" >
                <Card.Body>
                    <Form.Row>
                        ...
                        <Form.Group as={Col} controlId="formGridUsername">
                            ...
                            <Form.Control 
                                ...
                                onChange={this.withdrawChange}
                                className={"mb3"}
                                placeholder="Enter Withdraw username. Maximum 20 Characters" 
                                maxLength="20"
                            />
                        </Form.Group>
                    ...
                    <Form.Row>
                        <Form.Group as={Col} controlId="formGridwithdraw_date">
                            <Form.Label >Withdraw Date</Form.Label>
                            <Form.Control 
                                type="test"
                                name="withdraw_date"
                                value={withdraw_date}
                ...
                <Card.Footer className="text-right">
                    <Button type="summit" variant="success" onClick={this.props.match.params.username ? this.updateWithdraw : this.add_withdraw_button}>
                        {this.props.match.params.username ? "Edit Withdraw" : "Add Withdraw"}
                    </Button>
                    {" "}
